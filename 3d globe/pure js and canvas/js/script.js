...
const getRadius = (latitude) => {
    const yPart = Math.PI * 2;
    const radius = 600;
    const frame = map(latitude, 90, -90, 0, 1.65);
...
const getLandMassPaths = (name, radius, thickness) => {
    ...
    for (let i = 0; i < landmass.length; i++) {
        ...
        const p = latLongSphere(point.lat + $.scroll.lat, point.long + $.scroll.long, radius);
    ...
    landmass.forEach((point) => {
        const p = latLongSphere(point.lat + $.scroll.lat, point.long + $.scroll.long, radius);
        const p2 = latLongSphere(point.lat + $.scroll.lat, point.long + $.scroll.long, radius + thickness);
    ...
    const p = latLongSphere(landmass[0].lat + $.scroll.lat, landmass[0].long + $.scroll.long, radius);
    const p2 = latLongSphere(landmass[0].lat + $.scroll.lat, landmass[0].long + $.scroll.long, radius + thickness);
...
const closeCurve = (path, curveStart, p, radius) => {
    ...
    const a1 = getAngle({ x: 900, y: $.vCenter }, curveStart);
    const a2 = getAngle({ x: 900, y: $.vCenter }, p);
    const compare = a1 - a2;
...
const animateLoop = (time) => {
    ...
    const continentNames = ['southamerica', 'northamerica', 'greenland', 'japan', 'africa', 'australia', 'asia', 'indonesia', 'europe', 'britain', 'madagaskar', 'papua', 'nz'];
    const landPaths = [];
    const se = [];
...
const drawMarkers = (ctx, markers, drawFront) => {
    for (const marker of markers) {
        const ground = latLongSphere(marker.lat + $.scroll.lat, marker.long + $.scroll.long, 630);
        const needleTop = latLongSphere(marker.lat + $.scroll.lat, marker.long + $.scroll.long, 730);
